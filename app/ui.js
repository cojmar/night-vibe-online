...
export default class UI {
    ...
    selectPreset(presetId) {
        ...
        if (valuesToApply) {
            ...
            const targetClasses = classesToApply || JSON.parse(JSON.stringify(ConfigModule.FALLBACK_CLASS_DATA));
            ...
            const targetMonsters = monstersToApply || JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ENEMY_TYPES));
            ...
            const targetItems = itemsToApply || JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ITEMS_DB));
        ...
        if (btnNewPreset) {
            btnNewPreset.addEventListener('click', async () => {
                ...
                customPresets[newId] = {
                    ...
                    name: name.trim(),
                    values: valuesCopy,
                    classes: JSON.parse(JSON.stringify(ConfigModule.FALLBACK_CLASS_DATA)),
                    monsters: JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ENEMY_TYPES)),
                    items: JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ITEMS_DB))
                };
    ...
    resetToDefaultConfig() {
        ...
        Object.assign(ConfigModule.CLASS_DATA, JSON.parse(JSON.stringify(ConfigModule.FALLBACK_CLASS_DATA)));
        this.saveClassesToStorage();

        localStorage.removeItem('nightvibe-custom-monsters');
        ConfigModule.ENEMY_TYPES.length = 0;
        ConfigModule.ENEMY_TYPES.push(...JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ENEMY_TYPES)));
        this.saveMonstersToStorage();

        localStorage.removeItem('nightvibe-custom-items');
        ConfigModule.ITEMS_DB.length = 0;
        ConfigModule.ITEMS_DB.push(...JSON.parse(JSON.stringify(ConfigModule.FALLBACK_ITEMS_DB)));
        this.saveItemsToStorage();
    ...
    updateClassCarousel() {
        ...
        document.getElementById('stat-mp').innerHTML = `<strong style="color:#9b59b6">SPD (Speed): ${cd.spd}</strong><br><span style="color:#bdc3c7;font-size:0.9em;">Increases S2 AOE size, S2 cast duration, and reduces S2 cooldown.</span>`;
